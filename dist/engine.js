...
const url = `https://api.telegram.org/bot${String(token)}`;

function queryAPI(method, parameters = {}) {
...
*/
function sendResponse(method, parameters = {}) {
  let params = parametersToQueryString(parameters);
  let request = buildUrl(url, method, params);
...
function parametersToQueryString(params = {}) {
  let paramsString = Object.keys(params).map(k => {
    return `${k}=${params[k]}`;
  }).join("&");
...
function buildUrl(url, method, params) {
  if (params.length > 0) {
